...
- 除根节点外每个节点至少有`向上取整(m/2)`棵子树，若根节点不是终端阶段，至少有两个子树

- 每个节点最多有m个子树，m-1个关键字
- 所有叶节点在同一层

> 这里的叶节点是不带信息的挂载节点，终端节点是传统意义上的叶节点

### B+树

B+树叶节点包含全部键信息，非叶节点只起索引作用，会与叶节点中键重复

即为层层hash结构，一般包含一个指向根节点的指针和指向最小值的指针，可实现从根节点查找和从最小值开始遍历

**B/B+树广泛应用于操作系统的文件索引和数据库索引**